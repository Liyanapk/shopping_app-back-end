...
export const listProduct = async (req, res, next) => {
    try {
        ...
        if (req.query.catagory) {
            const catagoryDoc = await Catagory.findOne({ name: req.query.catagory });
            if (catagoryDoc) {
                filter.catagory = catagoryDoc._id;
            } else {
        ...
        const allProduct = await Product.find(filter)
            ...
            .skip(skip)
            .limit(limit)
            .populate("catagory") // Ensure your schema has `catagory` as a ref to Category
